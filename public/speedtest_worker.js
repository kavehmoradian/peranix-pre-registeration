...
function getIp(done) {
	...
	xhr.open("GET", settings.url_getIp + url_sep(settings.url_getIp) + "cors=true&" + (settings.getIp_ispInfo ? "isp=true" + (settings.getIp_ispInfo_distance ? "&distance=" + settings.getIp_ispInfo_distance + "&" : "&") : "&") + "r=" + Math.random(), true);
	...
	var testStream = function (i, delay) {
		setTimeout(
			function () {
				...
				xhr[i].open("GET", settings.url_dl + url_sep(settings.url_dl) + "cors=true&" + "r=" + Math.random() + "&ckSize=" + settings.garbagePhp_chunkSize, true); // random string to prevent caching
			...
			setTimeout(
				function () {
					...
					if (ie11workaround) {
						...
						xhr[i].open("POST", settings.url_ul + url_sep(settings.url_ul) + "cors=true&" + "r=" + Math.random(), true); // random string to prevent caching
						try {
...
function pingTest(done) {
	...
	var doPing = function () {
		...
		xhr[0].open("GET", settings.url_ping + url_sep(settings.url_ping) + "cors=true&" + "r=" + Math.random(), true); // random string to prevent caching
		xhr[0].send();
	}.bind(this);
...
function sendTelemetry(done) {
	...
	xhr.open("POST", settings.url_telemetry + url_sep(settings.url_telemetry) + "cors=true&" + "r=" + Math.random(), true);
